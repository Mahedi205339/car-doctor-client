...
const BookingTable = ({ booking, handleDelete, handleConfirm }) => {
    const { img, date, customerName, service, email, price, _id , status } = booking;
            ...
            <th>
                {
                    status === 'confirm' ? <span className="font-bold">Confirmed</span>
                        :
                        <button onClick={() => handleConfirm(_id)} className="btn btn-outline btn-ghost btn-xs">Confirm</button>
                }
            </th>
        </tr>
    );
